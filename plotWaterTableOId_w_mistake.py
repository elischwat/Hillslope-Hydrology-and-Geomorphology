...
def plotWaterTable(hilltype, curvature, Td):
    ...
    x_crit = (B/a)*(np.exp(-a*k*Tr) - 1)
    
    # Domain 1:
    ep = np.linspace(0,L,xRes/2)
    x1 = (ep + B/a)*np.exp(-a*k*Tr) - B/a
    h1 = (N/(k*(B + a*x1)*(b + ci*x1)))*(b*(ep-x1) + 0.5*ci*(ep**2 - x1**2))
    
    # Domain 2:
    tau = np.linspace(Tr,0,xRes/2) # flip these around so they are same order as others
    x2 = (B/a)*(np.exp(a*k*(tau - Tr)) - 1)
    h2 = -N*(b*x2 + 0.5*ci*x2**2)/(k*(B + a*x2)*(b + ci*x2))
    
    # Domain 3:
    x_star = np.linspace(0,x_crit,xRes/2)
    x3 = (x_star + B/a)*np.exp(a*k*(Tr-(Tr+Td))) - B/a
    h3 = (1/((B + a*x3)*(b + ci*x3)))*(
            (Nd/k)*(b*(x_star - x3) + 0.5*ci*(x_star**2 - x3**2)) +
            h2*(B + a*x_star)*(b + ci*x_star))
    
    # Domain 4:
    x_prime = np.linspace(x_crit,L,xRes/2)
    x4 = (x_prime + B/a)*np.exp(a*k*(Tr-(Tr+Td))) - B/a
    h4 = (1/((B+a*x4)*(b+ci*x4)))*(
            (Nd/k)*(b*(x_prime - x4) + 0.5*ci*(x_prime**2 - x4**2)) + 
            h1*(B + a*x_prime)*(b + ci*x_prime))
